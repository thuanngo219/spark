...
export type ContentRun = { text: string; bold?: true; italic?: true; underline?: true; softBreak?: true };
...
function sameMarks(a: ContentRun, b: ContentRun) {
  return a.bold === b.bold && a.italic === b.italic && a.underline === b.underline && a.softBreak === b.softBreak;
}
...
export function contentRuns(description: string | null, format: unknown): ContentRun[] {
  ...
  for (const entry of format) {
    ...
    runs.push({ text: entry.text, ...(entry.bold === true && { bold: true }), ...(entry.italic === true && { italic: true }), ...(entry.underline === true && { underline: true }), ...(entry.softBreak === true && entry.text === "\n" && { softBreak: true }) });
  }
...
export function normalizeContent(description: string | null, format?: unknown): ItemContent {
  ...
  return {
    ...
    descriptionFormat: runs.some((run) => run.bold || run.italic || run.underline || run.softBreak) ? mergeRuns(runs) : null,
  };
}
...
export function contentToDocument(description: string | null, format?: unknown): JSONContent {
  ...
  for (const run of runs) {
    ...
    if (run.softBreak) { paragraphs.at(-1)!.content!.push({ type: "hardBreak" }); continue; }
    run.text.split("\n").forEach((text, index) => {
