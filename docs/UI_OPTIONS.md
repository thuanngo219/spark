...
- Quick-add composer giữ checkbox Ghi chú hiện tại. Cả task và note có secondary button **Thêm Nội dung** để mở field “Nội dung (nếu cần)”. Textarea Nội dung khi tạo mới cao `160px` trên cả desktop và mobile. Ngày và dự án luôn khả dụng cho cả hai loại. Các overlay dùng lớp nền Navy dim đủ đậm để giữ bối cảnh nhìn thấy rõ, không blur nội dung phía sau.
...
- Item row: task dùng checkbox; note đang hoạt động dùng rounded dash mark xám có thể bấm để lưu trữ, note đã lưu trữ dùng icon archive để khôi phục. Cả hai loại có description thì hiển thị icon hội thoại nhỏ ngay sau Tên; icon chỉ báo trạng thái và tap row mở detail sheet. Canvas liệt kê trên desktop dùng chiều rộng responsive `940–1200px` (`80vw` trong dải này) và co theo vùng nội dung khả dụng khi màn hình hẹp; detail sheet desktop giữ chiều rộng `780px`, chỉ giữ nút X ở header, và ưu tiên trạng thái đọc với edit icon neo ở mép phải từng khối Tên/Nội dung. Khi edit Tên/Nội dung, action ✓ và × chuyển lên hàng nhãn, căn phải, để field dùng trọn chiều rộng; desktop dùng control `28px`/icon `16px`, mobile giữ touch target `44px`. Textarea Nội dung task/note khi edit cao `300px` trên desktop và `160px` trên mobile. URL trong Nội dung dùng link gạch chân, mở tab mới. Metadata ngày/dự án/Quan Trọng/Ưu tiên được gom gọn; trên mobile, Lưu trữ và Xóa là cặp icon-only 44px căn phải ở hàng riêng sau Dự án, ngăn bằng divider nhẹ. Xóa giữ màu destructive và luôn mở xác nhận trước khi thực hiện. Marker và project dot căn theo dòng title thay vì giữa toàn row. Desktop sidebar expanded cho drag project để đổi `position`; row nguồn giảm opacity và drop target hiện đường Turquoise kèm chấm tròn ở cạnh trên/dưới đúng vị trí chèn. Mobile không drag.
