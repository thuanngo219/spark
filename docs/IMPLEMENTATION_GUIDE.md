...
- Project dot trong item row là control tùy chọn: click/tap một dot đổi chung `projectLabelsExpanded` cho mọi item trong view. Pill uppercase dùng regular weight, tham gia grid, không overlay hoặc dùng shadow. Grid track dùng chung `--project-chip-width` từ pill dài nhất đang render để title thẳng hàng; từng pill dùng `--project-pill-width` riêng để nền chỉ ôm tên của nó, còn dot được neo cố định khi animate `width`. Trạng thái mở và toggle trong bảng Phím tắt & hiển thị đều lưu bằng `localStorage` theo browser, tiếp tục áp dụng khi đổi view; khi tắt tính năng, dot trở lại marker không tương tác và trạng thái mở được reset.
