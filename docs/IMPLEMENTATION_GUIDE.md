...
- Khung desktop đang sửa Nội dung cao `min(86dvh, 820px)`, bằng giới hạn khung đọc dài; flex phân bổ vùng nhập còn lại và chỉ editor cuộn. Quick-add giữ 160px; mobile edit dùng 40dvh. Nền edit dùng field cố định, quick-add focus dùng control-hover; Tên chỉ đổi màu border nhẹ, không glow/shadow. Trong edit Tên/Nội dung, đặt action ✓ và × trong `.detail-field-heading` cạnh nhãn, còn `.detail-inline-editor` dùng toàn chiều rộng bên dưới. Action control là `28px`/icon `16px` trên desktop; mobile giữ touch target `44px`/icon `18px`.
...
- Hàng nhãn Tên/Nội dung cao 28px ở cả chế độ đọc/sửa: desktop cách field 4px, mobile cách 8px. Nút mobile vẫn có vùng chạm 44px, dùng margin dọc -8px để không làm hàng nhãn cao thêm; vùng chạm kết thúc trước field bên dưới.
