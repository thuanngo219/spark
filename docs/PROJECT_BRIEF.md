...
- Khung sửa Nội dung desktop dùng chiều cao tối đa như khung đọc nội dung dài: `min(86dvh, 820px)`, vùng nhập giãn theo phần còn lại. Quick-add giữ `160px`; mobile edit dùng vùng nhập `40dvh`. Trong edit Tên/Nội dung, action ✓ và × nằm trên hàng nhãn để field dùng trọn chiều rộng; control desktop là `28px` với icon `16px`, mobile giữ touch target `44px`. Hàng nhãn Tên/Nội dung cao 28px ở cả trạng thái đọc/sửa; mobile đặt Dự án và Lưu trữ/Xóa cùng một hàng, không tràn ngang ở 320px.
